...
test("Active wildfires layer", async ({ page }) => {
  ...
  let src = await legend
    .locator('a:text-is("data services")')
    .getAttribute("href");
...
test("Hotspots layer", async ({ page }) => {
  ...
  src = await legend
    .locator('a:text-is("NOAA Active Fire Detections from the VIIRS sensor")')
    .getAttribute("href");
...
test("Current air quality", async ({ page }) => {
  ...
  let src = await legend
    .locator('a:text-is("Air Quality Index (AQI)")')
    .getAttribute("href");
...
test("Footer links", async ({ page }) => {
  ...
  let src = await page
    .locator('.footer a:text-is("Alaska Interagency Coordination Center")')
    .first()
    .getAttribute("href");
  expect(src).toContain("https://fire.ak.blm.gov/aicc.php");

  src = await page
    .locator('.footer a:text-is("MesoWest data services")')
    .getAttribute("href");
  ...
  src = await page
    .locator('.footer a:text-is("PurpleAir")')
    .getAttribute("href");
  expect(src).toContain("https://www2.purpleair.com/");

  src = await page
    .locator('.footer a:text-is("Geographic Information Network of Alaska")')
    .getAttribute("href");
  ...
  src = await page
    .locator('.footer a:text-is("Alaska Climate Adaptation Science Center")')
    .getAttribute("href");
  ...
  src = await page
    .locator('.footer a:text-is("Center for Disaster Resilient Communities")')
    .getAttribute("href");
  ...
  src = await page
    .locator('.footer a:text-is("International Arctic Research Center")')
    .getAttribute("href");
  expect(src).toContain("https://uaf-iarc.org/");

  src = await page
    .locator('.footer a:text-is("University of Alaska Fairbanks")')
    .getAttribute("href");
  expect(src).toContain("https://uaf.edu/uaf/");

  src = await page
    .locator('.footer a:text-is("uaf-snap-data-tools@alaska.edu")')
    .getAttribute("href");
  expect(src).toContain("mailto:uaf-snap-data-tools@alaska.edu");

  src = await page
    .locator('.footer a:text-is("Visit the SNAP Climate + Weather Tools page")')
    .getAttribute("href");
  expect(src).toContain("https://uaf-snap.org/tools-overview/");

  src = await page
    .locator('.footer a:text-is("University of Alaska")')
    .getAttribute("href");
  expect(src).toContain("https://www.alaska.edu/");

  src = await page
    .locator('.footer a:text-is("policy of non-discrimination")')
    .getAttribute("href");
